...
#'
#' @description 
#' This function will multiply the input values of X and Y, put description of function 
#' 
...
# Now let's make a list of filepaths to each peak file -- "list.files" prints list names in folder?
...
fl_ep300 <- fl[grep("CHD2", fl)]
fl_ep300 # check only have 2 files- bc only 2 replicates 
...
y <-  str_extract(fl[7], "([^\\/]+$)")    # this will get rid of nasty name to get rid of all stuff and will rename the file to just have the protein name to make it easier 
...
unlist(strsplit(y, "_"))[[1]] #breaking up underscore just want the first underscore so should get new name that stops before the first _ which will just give you the name 

# what happens if we try and run together: extracting string of characters 

y <-  str_extract(fl[7], "([^\\/]+$)") %>%
  unlist(strsplit(y, "_"))[[1]] 

#y doesnt really exist- something is being done to y- cant call it again 

# Error !! Why ?
# This is because y is not a local variable. y is global( in global environment to right) and not set.
...
#runs function for all names to correct them 
