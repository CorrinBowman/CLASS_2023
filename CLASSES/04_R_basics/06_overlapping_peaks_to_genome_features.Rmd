...
path <- "2023_2_10_group3/group/results/bwa/mergedLibrary/macs/broadPeak"

# first we read the peak files in as gRanges object with rtracklayer function.
peaks1 <- rtracklayer::import(file.path(basepath,path, "CEBPZ_R1_peaks.broadPeak"))
peaks2 <- rtracklayer::import(file.path(basepath,path, "CEBPZ_R2_peaks.broadPeak"))
...
length(peaks1) # check the size of the peaks- smaller than the data- couple classes ago why 

# nice we see that 19,063 promoters overlap with Pol2 replicate 1.- for my data its 339 overlaps bc peak can b large and overlap multiple peaks- big peak can lead to multiple overlaps
...
#109-120 DO FOR PROJECT EACH PROTEIN, EACH REPLICATE 
# indexing
