...
# # BiocManager::install("circlize")
 library(circlize)
 source("/scratch/Shares/rinnclass/CLASS_2023/corrinbowman/CLASS_2023/util/intersect_functions.R")
...
peak_coverage <- coverage(consensus_peaks[["CEBPZ"]]) 
...
peak_coverage <- peak_coverage[chromosomes] #now we index into the chromosomes

#chromosomes are all the ones that have peak coverage 
...
# using base R here with 'as' a coercing function. need in integer ranges list- so its same structure as peak coverage, only take chromosomes represented which we did above chromosomes variable= chromosomes w peak coverage
...
# let's look at views
?Views
# ok so Views can store and access all the promoter vectors, kinda like find overlaps 
# We will end up with an index of all 'promoter windows' and peak ranges

# >>> Thus we end up with , eliminates peak coverage(chromosome) not bound to promoter
promoter_peak_view <- Views(peak_coverage, all_promoters_ir)

#let's look at what we got
promoter_peak_view
# Note that these are still in run-length encoding format.
# all 2000 wide because thts our window we made, now hav ~3k overlaps 
```
...
# the t is for transmute that turns rows into cols, then make into vector, we just flipping this around, lapply like for loop to go thru all chromosomes 
...
# Let's look (note rows turned into cols)- no rows just read as columns
promoter_peak_view
...
# now we have a matrix of nrow_promoters by 6,000 cols, looking at length dimensions, should have all overlaps except the ones on end we trimmed off above  
...
# taking only minus strand rows promoter, making it an object, going to flip it so dna matches lines up
...
#start at end of column ?

# Now we can get rid of the rows that have no peaks- no coverage, summing rows to see if theres a 1 in the row it will be greater than zero and then that means we want to keep it, continuing to cut down and filter data
...
# sum of cols (colSums) = how many peak overlaps there were at each base (cols). sum the number of 1s and then normalize against sum 

peak_sums <- colSums(promoter_peak_matrix) # how many 1s on the start site, summing the overlaps and then going to divide by the total amount of promoters 

# normalization of binding events to 1 across promoter window, avg of density w overlap, sum of column of 'dens'=1 to be 100%
peak_dens <- peak_sums/sum(peak_sums)

# Create a data frame in order to plot this, making obbject to plot
...
# Nice, HDAC2 has normal distribution of binding events-               i did this for CEBPZ!!!
...
ggplot(metaplot_df, aes(x = x, y = dens)) + 
  ...
  scale_x_continuous(breaks = c(-3000, 0, 3000),
                     ...
                     name = "") + 
  ylab("Peak frequency")
ggsave("/scratch/Shares/rinnclass/CLASS_2023/corrinbowman/CLASS_2023/CLASSES/05_R_analyses/04_metaplots/figures/CEBPZ_promoter_metaplot.pdf")
```
