...
#these are his proteins, need to put my names in 
# HEP300
...
#dev.off() do this if plot is not showing up 

#my proteins 

plot(profile_tss(consensus_peaks[["CEBPZ"]], lncrna_mrna_promoters))
plot(profile_tss(consensus_peaks[["CHD2"]], lncrna_mrna_promoters))
plot(profile_tss(consensus_peaks[["CTCF"]], lncrna_mrna_promoters))
plot(profile_tss(consensus_peaks[["ELF1"]], lncrna_mrna_promoters))
plot(profile_tss(consensus_peaks[["EP300"]], lncrna_mrna_promoters))
...
test <- profile_tss(consensus_peaks[["CEBPZ"]], lncrna_mrna_promoters)
...
write_rds(metaplot_df, "/scratch/Shares/rinnclass/CLASS_2023/corrinbowman/CLASS_2023/CLASSES/05_R_analyses/04_metaplots/results/metaplot_df_final.rds")
...
# If there are any NAs Let's filter out those is.na- i didnt have any
 metaplot_filtered_matrix <- metaplot_filtered_matrix[rowSums(is.na(metaplot_filtered_matrix)) == 0,]

# Z-Scale the rows for clustering (not needed for profile plots), need to turn rows along column scaling then flipping back to rows think
...
dev.off()
```
clustering based on similarity of profiles
...
#our heatplot wont work dont worry about it
...
```
makes a graph of all of them together instead of 10 graphs separate 
