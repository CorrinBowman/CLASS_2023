...
---
annotations- feature of genome, example gene- then gene has many annotations like exons 
...
peak_path <- "CLASS_2023/CLASSES/06_final_going_global/analysis/01_create_consensus_peaks/results/Consensus Peaks"
#05_R_analyses/00_consensus_peaks/Consensus Peaks
...
names(consensus_peaks) <- gsub("/scratch/Shares/rinnclass/CLASS_2023/corrinbowman/CLASS_2023/CLASSES/06_final_going_global/analysis/01_create_consensus_peaks/results/Consensus Peaks/|_consensus.bed","", consensus_peaks_files)
...
# creating and exporting promoter annotations, making window upsteam downstream of width 
...
#where ever consensus peaks- change to consesnsu_list 
...
# counting promoter overlaps, giving us the function in this count peak per feature file have to source his file 
source ("../../../util/my_class_functions.R")
...
# so now we just row sum for each DBP ! can know how many promoters overlapped total 

num_peaks_df$peaks_overlapping_promoters <- rowSums(promoter_peak_counts)
# when there is more promoter overlaps than total peaks it is becasue- protein likes to bind w promoters, peak are mostly overlapping promoters, the start of a promoter will never have a nucleosome it has a nuclesome free regions- so they are separated and counts it as 2 peaks- counting 2 as 1 promoters can overlap, as promoter window increases you get more overlaps- think theres more promoters in that window
# look to see if genes are supposed to be overlapping promoters or not then see if the results show that
...
# All gene bodies- looking to see if the overlaps are in the gene body 
# if gene not rly binding promoters then look to see should be binding more of gene body 
# can compare different rna in table which binds more 
...
#save a bunch of these now- 

save(num_peaks_df, file = "results/num_peaks_df.RData")
save(lncrna_genes, file = "results/lncrna_genes.RData")
save(lncrna_mrna_genes, file = "results/lncrna_mrna_genes.RData")
save(lncrna_mrna_genes_df, file = "results/lncrna_mrna_genes_df.RData")
save(lncrna_mrna_promoters, file = "results/lncrna_mrna_promoters.RData")
save(mrna_lncrna_genes, file = "results/mrna_lncrna_genes.RData")
save(peak_num, file = "results/peak_num.RData")
save(mrna_genes, file = "results/mrna_genes.RData")
...
however they are never perfect :) # looking for transcription factors- looking at which proteins are TF or not
...
destination_for_url <- "/scratch/Shares/rinnclass/CLASS_2023/corrinbowman/CLASS_2023/CLASSES/06_final_going_global/analysis/01_create_consensus_peaks/results/TF/TF_annotations.xlsx" #excel file
...
# have multiple sheets in excel we only want sheet 2 and skip first line 
#redx1::read_excel to import
human_tfs <- readxl::read_excel("results/TF/TF_annotations.xlsx",
                                sheet = 2, skip = 1)
...
# now let's intersect gene names that are in our ChIP data and has TF identity, making all words lowercase so when it searches they match- mathicng the hum tf file to our gene names, askinf how many genes that are ours- are in the dta set- all of min are 5/5
...
# first let's filter and grab the first 4 columns that match DBPs in num_peaks_df, reducing column to 4  
...
#merging the new tf tabke info with our num peaks file 
...
write_csv(num_peaks_df, "/scratch/Shares/rinnclass/CLASS_2023/corrinbowman/CLASS_2023/CLASSES/06_final_going_global/analysis/01_create_consensus_peaks/results/num_peaks_df.csv")

save(num_peaks_df, file = "results/num_peaks_df.RData")
save(human_tfs, file = "results/human_tfs.RData")
...
save(peak_occurence_df, file = "results/peak_occurence_df.RData")
...
save(consensus_list, gencode_genes, lncrna_gene_ids, mrna_gene_ids, num_peaks_df, peak_occurence_df, lncrna_mrna_promoters, mrna_lncrna_genes, file = "results/peak_features.RData")
